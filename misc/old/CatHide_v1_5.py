...
class CatHidePresetUIList(bpy.types.UIList):
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname):
        if bpy.context.scene.cathide_startup_preset == -1:
            layout.prop(item, "name", text="", icon='SETTINGS',  emboss=False, translate=False)
        else:
            layout.prop(item, "name", text="", icon='SETTINGS',  emboss=False, translate=False)
            if item.startup==True:
                layout.label(icon='LINK_BLEND')
        ...
        if scene.cathide_show_preset==True:
            ...
            if len(prlist)>=1 and (idx+1) <= len(prlist):
                ...
                if scene.cathide_show_presetsutility==True:
                    row = layout.row()
                    row.menu("CatHide_CopyPreset_ToScene", text='Copy to', icon='COPY_ID')
                    row.prop(scene, "cathide_copyallpresets", text='All Presets')
                    row = layout.row()
                    if scene.cathide_startup_preset == -1 or nostartup==1:
                        row.operator("call.createstartup_menu", text='Create Startup Preset', icon='LINK_BLEND')
                    elif nostartup==0:
                        row.operator("cathide.update_startuppreset")
                        row.operator("cathide.clear_startuppreset", icon='X')
...
# Create Startup Preset
class CathideCreateStartupPreset(bpy.types.Operator):
    bl_idname = "cathide.create_startuppreset"
    bl_label = "Create Startup Preset"
    bl_description = "Create Startup Preset"
    bl_options = {'REGISTER', 'UNDO'}
        
    def execute(self, context):
        if bpy.context.scene.cathide_preset_index >= len(bpy.context.scene.cathide_preset_list):
            info = 'Please select a CatHide Preset'
            self.report({'ERROR'}, info)
        else:
            chk=0
            for t in bpy.data.texts:
                if t.name=="___cathide startup script___.py":
                    text=t
                    chk=1
            if chk==0:
                text=bpy.data.texts.new("___cathide startup script___.py")
            elif chk==1:
                text.clear()
            text.write("import bpy\n")
            text.write("startup_preset=bpy.context.scene.cathide_startup_preset\n")
            text.write("if startup_preset != -1 and len(bpy.context.scene.cathide_preset_list) >= (startup_preset+1):\n")
            #indent
            text.write("    bpy.context.scene.cathide_preset_index=startup_preset\n")
            text.write("    if bpy.context.scene.cathide_preset_list[bpy.context.scene.cathide_preset_index].startup==True:\n")
            #indent
            text.write("        bpy.ops.cathide.applypreset()\n")
            text.write("        print('Cathide Startup Preset applied')")
            text.use_module=True
            bpy.context.scene.cathide_startup_preset=bpy.context.scene.cathide_preset_index
            for i in bpy.context.scene.cathide_preset_list:
                i.startup=False
            bpy.context.scene.cathide_preset_list[bpy.context.scene.cathide_preset_index].startup=True
    
        return {'FINISHED'}
    
# Clear Startup Preset
class CathideClearStartupPreset(bpy.types.Operator):
    bl_idname = "cathide.clear_startuppreset"
    bl_label = "Clear Startup Preset"
    bl_description = "Clear Startup Preset"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        bpy.context.scene.cathide_startup_preset=-1
        for i in bpy.context.scene.cathide_preset_list:
                i.startup=False
    
        return {'FINISHED'}
    
# Update Startup Preset
class CathideUpdateStartupPreset(bpy.types.Operator):
    bl_idname = "cathide.update_startuppreset"
    bl_label = "Update Startup Preset"
    bl_description = "Update Startup Preset"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        chk=0
        for t in bpy.data.texts:
            if t.name=="___cathide startup script___.py" and bpy.context.scene.cathide_startup_preset!=-1:
                chk=1
        if chk==1:
            bpy.context.scene.cathide_startup_preset=bpy.context.scene.cathide_preset_index
            for p in bpy.context.scene.cathide_preset_list:
                p.startup=False
            bpy.context.scene.cathide_preset_list[bpy.context.scene.cathide_preset_index].startup=True
...
# Menu Validation Create Startup Preset
class CatHideCreateStartupMenu(bpy.types.Menu):
    bl_label = "Create Startup Preset"
    bl_idname = "Menu_CreateStartup_CatHidePresets"
    
    def draw(self, context):
        layout = self.layout
        layout.label("After validating, you have to save the currend Blend File for Startup Preset to work", icon='ERROR')
        layout.label("Auto run Python Scripts have to be Enabled in User Preferences")
        layout.operator("cathide.create_startuppreset",text="Click to Create Startup Preset")

# Call Validation Create Startup Preset
def callcreatestartupmenu (context):
    bpy.ops.wm.call_menu(name=CatHideCreateStartupMenu.bl_idname)
    
class CatHideCallCreateStartupMenu(bpy.types.Operator):
    """Create Startup Preset"""
    bl_idname = "call.createstartup_menu"
    bl_label = "Call Create Startup Menu"

    def execute(self, context):
        callcreatestartupmenu(context)
        return {'FINISHED'} 
...
class CatHidePresetList(bpy.types.PropertyGroup):
    ...
    startup = bpy.props.BoolProperty(name="startup", default = False)
...
def register():
    ...
    bpy.utils.register_class(CathideCreateStartupPreset)
    bpy.utils.register_class(CathideClearStartupPreset)
    bpy.utils.register_class(CathideUpdateStartupPreset)
    ...
    bpy.utils.register_class(CatHideCreateStartupMenu)
    bpy.utils.register_class(CatHideCallCreateStartupMenu)
    ...
    bpy.types.Scene.cathide_startup_preset = IntProperty(default=-1)
...
def unregister():
    ...
    bpy.utils.unregister_class(CathideCreateStartupPreset)
    bpy.utils.unregister_class(CathideClearStartupPreset)
    bpy.utils.unregister_class(CathideUpdateStartupPreset)
    ...
    bpy.utils.unregister_class(CatHideCreateStartupMenu)
    bpy.utils.unregister_class(CatHideCallCreateStartupMenu)
    ...
    del bpy.types.Scene.cathide_startup_preset
